...
CHANNEL_FILE="$REPO_DIR/channel-vocab.json"
FEED_FILE="$REPO_DIR/hsk-vocab-feed.xml"

if [ ! -f "$CHANNEL_FILE" ]; then
  echo "Error: channel-vocab.json not found at $CHANNEL_FILE"
  exit 1
fi
...
while IFS= read -r -d '' file; do
  EPISODE_FILES+=("$file")
done < <(find "$REPO_DIR/vocab-episodes" -name "metadata.json" -print0 2>/dev/null)
...
for entry in "${SORTED_EPISODES[@]}"; do
  ...
  CHAPTERS_URL="${LINK}vocab-episodes/${SLUG}/chapters.json"
  
  # Check if chapters.json exists
  CHAPTERS_FILE="$REPO_DIR/vocab-episodes/${SLUG}/chapters.json"
  HAS_CHAPTERS="false"
