...
if [ -z "$YOUTUBE_URL" ]; then
  echo "Usage: bash scripts/add-vocab-episode.sh <youtube-url> [chapter-file|-]"
  echo ""
...
# For HSK vocab content, try to extract a meaningful slug
...
# Prefix with vocab- if not already
if [[ ! "$SLUG" =~ ^vocab- ]]; then
  SLUG="vocab-${SLUG}"
fi
...
echo ""
echo "=========================================="
echo "Vocabulary episode created successfully!"
echo "=========================================="
echo "Slug: $SLUG"
...
echo ""
echo "Next steps:"
echo "  1. Review the episode in vocab-episodes/$SLUG/"
echo "  2. Run: bash scripts/publish-vocab-release.sh $SLUG"
echo "=========================================="
