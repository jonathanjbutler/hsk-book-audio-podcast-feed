...
AUDIO_DIR="$REPO_DIR/audio-vocab"
EPISODES_DIR="$REPO_DIR/vocab-episodes"

# --- Argument parsing ---
SLUG="${1:-}"

if [ -z "$SLUG" ]; then
  echo "Usage: bash scripts/publish-vocab-release.sh <episode-slug>"
  echo ""
  echo "Available vocabulary episodes:"
...
if [ ! -f "$AUDIO_FILE" ]; then
  ...
  echo "Run add-vocab-episode.sh first to download and prepare the audio."
  exit 1
fi
...
# --- Regenerate feed ---
echo "Regenerating hsk-vocab-feed.xml..."
bash "$SCRIPT_DIR/generate-vocab-feed.sh"
...
cd "$REPO_DIR"
git add -A
git commit -m "Add vocabulary episode: $TITLE

- Audio uploaded to GitHub Release: $SLUG
- Vocab feed updated with episode metadata
...
echo ""
echo "=========================================="
echo "Vocabulary episode published!"
echo "=========================================="
echo "Release: https://github.com/jonathanjbutler/hsk-book-audio-podcast-feed/releases/tag/$SLUG"
echo "Feed: https://jonathanjbutler.github.io/hsk-book-audio-podcast-feed/hsk-vocab-feed.xml"
echo ""
echo "Subscribe in Overcast:"
echo "  https://jonathanjbutler.github.io/hsk-book-audio-podcast-feed/hsk-vocab-feed.xml"
echo "=========================================="
